...
# enable/disable ssl based on env variable set from docker container run command
 if [[ $SSL == "yes" ]]; then
        ...
        /opt/madsonic/madsonic.sh --home=/config --context-path=/mad --host=0.0.0.0 --https-port=4050 --default-music-folder=/media --default-podcast-folder=/config/media/podcast --default-playlist-import-folder=/config/playlists/import --default-playlist-export-folder=/config/playlists/export --default-playlist-backup-folder=/config/playlists/backup

 elif [[ $SSL == "no" ]]; then
        echo "Disabling SSL for Madsonic"
        /opt/madsonic/madsonic.sh --home=/config --context-path=/mad --host=0.0.0.0 --port=4040 --default-music-folder=/media --default-podcast-folder=/config/media/podcast --default-playlist-import-folder=/config/playlists/import --default-playlist-export-folder=/config/playlists/export --default-playlist-backup-folder=/config/playlists/backup
